...
clean_results_table <- gt(results_table, groupname_col = "method") 
...
clean_results_table <- cols_label(clean_results_table,
                                  chrX_num_dmrs = " #\nDMRs ", chrX_mean_width = " mean\nDMR width ",
                                  chrX_median_width = " median\nDMR width ", chrX_percent_total_width = " % total\nwidth ",
                                  autosome_num_dmrs = " #\nDMRs ", autosome_mean_width = " mean\nDMR width ",
                                  autosome_median_width = " median\nDMR width ", autosome_percent_total_width = " % total\nwidth " )
...
```{r}
X<- data.frame(cluster=c("X","X","X","X","X"), 
          chr=c("chrX","chrX","chrX", "chrX", "chrX"),
          start_pos=c(71e6, 71.25e6, 71.45e6, 71.45e6, 47.05e6),
          stop_pos=c(72e6, 71.75e6, 71.55e6, 71.55e6, 49.36e6))
B<-B[,c(g1,g2)]
...
displayPars(itrack) <- list(cex=cex_small,cex.bands=2)
  ...
  filename=paste("/home/lbondhus/Desktop/PROJECTS/dmrscaler_real_data/figures/SEX/", "chrX_" ,start, "_", stop, ".png", sep = "")
...
hc <- HilbertCurve(s=1,e=hc_max, level = 4, reference = F)
hc_points(hc, x1 = 1:nrow(hc_points), np = NULL, pch = 16, size = unit(hc_size*2, "mm"),gp = gpar(col = hc_col, fill = hc_col))
...
hc_col[which(hc_points$scoring_values>fdrscaler)] <- "red"
hc_size <- hc_points$scoring_values / max(hc_points$scoring_values)
hc <- HilbertCurve(s=1,e=hc_max, level = 5, reference = F)
hc_points(hc, x1 = 1:nrow(hc_points), np = NULL, pch = 16, size = unit(hc_size*2, "mm"),gp = gpar(col = hc_col, fill = hc_col))
