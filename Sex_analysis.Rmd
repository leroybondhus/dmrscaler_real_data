...
results_dir <-paste("/home/leroy/Desktop/PROJECTS/dmrscaler_real_data/results2/SEX/")
figures_dir <- paste("/home/leroy/Desktop/PROJECTS/dmrscaler_real_data/figures2/SEX/")
intermediate_data_dir <-paste("/home/leroy/Desktop/PROJECTS/dmrscaler_real_data/intermediate_data2/SEX/")
library(Gviz)
library(data.table)
```
...
```{r }
load("/home/leroy/Desktop/PROJECTS/dmrscaler_real_data/intermediate_data/With_X_Sotos_GRset.funnorm.Rdata")
...
save(built_layers, file=paste(intermediate_data_dir,"/Sex_8M_8F_built_layers.Rdata",sep=""))              
save(data, file=paste(intermediate_data_dir,"/Sex_8M_8F_atomic_layers.Rdata",sep=""))

load(file=paste(intermediate_data_dir,"/Sex_8M_8F_built_layers.Rdata",sep=""))              
load(file=paste(intermediate_data_dir,"/Sex_8M_8F_atomic_layers.Rdata",sep=""))

for(i in 1:length(built_layers)){
  write.csv(built_layers[[i]], paste(results_dir,"/dmrscaler_layer",i,".csv", sep = ""))
}
...
write.csv(dmrcate_dmrs_1M, paste(results_dir,"dmrcate_dmrs_1M.csv", sep = ""))
...
Sys.time()
print("start with 2nd combp")
system("comb-p pipeline -c 4 --dist 1000000 --step 100000 --seed 1e-3 -p ./intermediate_data2/test --region-filter-p 0.1 ./intermediate_data2/combp_temp_input.bed ")
Sys.time() ## takes 7hrs at dist=1mb step=100kb :0
## Parse comb-p resuults into the CALLED_TP_FP and the MAPPING VALUES tables
combp_results_1M_100k <- data.table::fread("./intermediate_data2/test.regions-t.bed")
write.csv(combp_results_1M_100k, paste(results_dir,"combp_results_1M_100k.csv", sep = ""))
...
df <- data.frame(lambda = character(), ratio = numeric(), C <-numeric(), percentile <- numeric(), width <- numeric() )
for(lambda in c(1e3,5e3,1e4,5e4,1e5,5e5,1e6)){
  for( r in c(500,1000,2000) ){
    C <- lambda/r

    dmrcate_results <- dmrcate(myannotation, lambda=lambda, C=C)
    dmrcate_results <- extractRanges(dmrcate_results , genome = "hg19")
    temp <- quantile(dmrcate_results@ranges@width, probs=seq(0,1,by=0.01))
    df <- rbind(df, data.frame( lambda=rep(lambda,length(temp)), ratio=rep(r,length(temp)), C=rep(C,length(temp)),
                                percentile = seq(0,1, by =0.01), width=temp))

    
  }
}
df$name <- paste("lambda=",df$lambda,"lambda/C=",df$ratio)
ggplot(df, aes(x=percentile, y=log10(width),color=as.factor(lambda) ))+
  geom_line()+
  facet_grid(cols = vars("lambda/C"=ratio))

ggplot(df, aes(x=percentile, y=log10(width),color=as.factor(ratio) ))+
  geom_line()+
  facet_grid(cols = vars("lambda/C"=lambda))




df2 <- data.frame(lambda = character(), ratio = numeric(), C <-numeric(), width <- numeric() )
for(lambda in c(1e3,5e3,1e4,5e4,1e5,5e5,1e6)){
  for( r in c(500,1000,2000) ){
    C <- lambda/r
    ...
    dmrcate_results <- dmrcate(myannotation, lambda=lambda, C=C)
    dmrcate_results <- extractRanges(dmrcate_results , genome = "hg19")
    temp <- dmrcate_results@ranges@width
    df2 <- rbind(df2, data.frame( lambda=rep(lambda,length(temp)), ratio=rep(r,length(temp)), C=rep(C,length(temp)),
                                width=temp))

  }
}
df2$name <- paste("lambda=",df$lambda,"lambda/C=",df2$ratio)

gg <- ggplot(df2, aes(x=log10(width)))+
  geom_histogram(aes(y = stat(..density..)))+
  facet_grid(cols = vars(lambda), rows = vars(ratio))


filename=paste("/home/leroy/Desktop/PROJECTS/dmrscaler_real_data/figures/Supplemental/DMRcate_Variable_Lambda_Sex.png", sep = "")
png(filename = filename, width = 10, height = 6, units = "in", res = 300)
grid.newpage()
...
which <- which(combp_results_1k_100$X.chrom == "chrX")
which_not <- which(combp_results_1k_100$X.chrom != "chrX" &
                     combp_results_1k_100$X.chrom != "chrY")
results_table <- rbind(results_table,
                         data.frame("method" = "combp", 
                         "chrX_num_dmrs" = nrow(combp_results_1k_100[which,]),
                         "chrX_mean_width" = mean(combp_results_1k_100$end [which]-combp_results_1k_100$start[which]),
                         "chrX_median_width"= median(combp_results_1k_100$end [which]-combp_results_1k_100$start[which]),                         
                         "chrX_percent_total_width" = -1,  # add later
                         "autosome_num_dmrs" = nrow(combp_results_1k_100[which_not,]),
...
rownames(results_table)[length(rownames(results_table))] <- "dist = 1 kb, step = 100 bp"

which <- which(combp_results_1M_100k$X.chrom == "chrX")
which_not <- which(combp_results_1M_100k$X.chrom != "chrX" & combp_results_1M_100k$X.chrom != "chrY")
results_table <- rbind(results_table,
                         data.frame("method" = "combp", 
                         "chrX_num_dmrs" = nrow(combp_results_1M_100k[which,]),
                         "chrX_mean_width" = mean(combp_results_1M_100k$end[which]-combp_results_1M_100k$start[which]),
                         "chrX_median_width"= median(combp_results_1M_100k$end[which]-combp_results_1M_100k$start[which]),                         
                         "chrX_percent_total_width" = -1,  # add later
                         "autosome_num_dmrs" = nrow(combp_results_1M_100k[which_not,]),
                         "autosome_mean_width" = mean(combp_results_1M_100k$end[which_not]-combp_results_1M_100k$start[which_not]),
                         "autosome_median_width"= median(combp_results_1M_100k$end[which_not]-combp_results_1M_100k$start[which_not]),                            
                         "autosome_percent_total_width" = -1 # add late
                         ))
rownames(results_table)[length(rownames(results_table))] <- "dist = 1 Mb, step = 100 kb"
...
gtf_df <- rtracklayer::import("/home/leroy/Desktop/STABLE_DATA/HUMAN_GENOME/Homo_sapiens.GRCh37.87.chr.gtf")
...
displayPars(itrack) <- list(cex=1,cex.bands=1, fontcolor="grey20")
  ...
  #filename=paste(figures_dir, "/KAT6A/", Region_name , ".png", sep = "")
  filename=paste(figures_dir, "FINALv2_", chr,"_" ,start, "_", stop, ".png", sep = "")
  ...
  level = 2
  ## 4^level - 1 = # segments
  filename=paste(figures_dir"test1_Adjacent_CG_plot_", chr,"_" ,start, "_", stop, ".png", sep = "")  
...
write(jsonData, paste(intermediate_data_dir,"/sex_dmr_tree_output.json",sep=""))
#write(jsonData, intermediate_data_dir,"/sex_dmr_tree_output_full.json",sep="))
...
saveNetwork(temp,paste(figures_dir,"All_chrom_dmrs_radialNetwork.html",sep = "") ,
            selfcontained = T)
webshot(paste(figures_dir,"All_chrom_dmrs_radialNetwork.html",sep = ""),
        paste(figures_dir,"All_all_layer_chrom_dmrs_radialNetwork.png",sep = ""),
        zoom = 2)
...
png(paste(figures_dir, "X_chrom_network_plot.png",sep=""), height = 1000, width = 1000 )
...
png(paste(figures_dir, "direction_Hilbert_curves_chrX.png",sep=""), width = 3, height=3, units = "in", res=400)
...
tiff(paste(figures_dir, "Hilbert_curves_chrX.tiff",sep=""), width = 3, height=3, units = "in", res=400)
...
png(paste(figures_dir, "Hilbert_curves_chrX_inv.png",sep=""), width = 600, height=600)
...
dev.off()
png(paste(figures_dir, "Hilbert_curves_chr1_inv.png",sep=""), width = 600, height=600)
...
for(i in 1:length(built_layers)){
  ...
  filename=paste(figures_dir, "/Supplemental/DMRscaler_SEX_width_distr_layer",i, ".png", sep = "")  
...
filename=paste(figures_dir, "/Supplemental/bumphunter_default_SEX_width_distr.png", sep = "")  
png(filename = filename, width = 10, height = 6, units = "in", res = 150)
grid.newpage()
grid.arrange(ggSex, ggAuto, nrow=1)
dev.off()  
...
#combp 
which <- which(combp_results_1M_100k$X.chrom == "chrX")
df <- data.frame(widths = combp_results_1M_100k$end[which] - combp_results_1M_100k$start[which])
ggSex <- ggplot(df, aes(x=log10(widths)) )+
  ggtitle(label = "X chromosome DMR widths",
          subtitle = paste("combp dist = 1 Mb,step = 100 kb", sep=""))+
  geom_histogram(bins=50)+
  xlim(0,8.5)+
  theme(text=element_text(size=18))

which <- which(combp_results_1M_100k$X.chrom != "chrX" & combp_results_1M_100k$X.chrom != "chrY"  )
df <- data.frame(widths = combp_results_1M_100k$end[which] - combp_results_1M_100k$start[which])
ggAuto <- ggplot(df, aes(x=log10(widths)) )+
  ggtitle(label = "Autosome widths",
          subtitle = paste("combp dist = 1 Mb,step = 100 kb", sep=""))+
...
filename=paste(results_dir, "/combp_1Mb_100kb_SEX_width_distr.png", sep = "")  
png(filename = filename, width = 10, height = 6, units = "in", res = 150)
grid.newpage()
grid.arrange(ggSex, ggAuto, nrow=1)
dev.off()  
...
filename=paste(figures_dir, "/Supplemental/DMRcate_1Mb_SEX_width_distr.png", sep = "")
png(filename = filename, width = 10, height = 6, units = "in", res = 150)
grid.newpage()
grid.arrange(ggSex, ggAuto, nrow=1)
dev.off()  
...
which <- which(combp_results_1k_100$X.chrom == "chrX")
temp <- quantile(log10(combp_results_1k_100$end[which] - combp_results_1k_100$start[which]), seq(0.01,1,by=0.01), type=1)
quants <- rbind(quants, data.frame(percentile = seq(0.01,1,by=0.01), value = temp,
                                      method = "combp",pars="combp:dist=1kb,step=100bp", chrs="chrX"))
which <- which(combp_results_1k_100$X.chrom != "chrX" & combp_results_1k_100$X.chrom != "chrY" )
temp <- quantile(log10(combp_results_1k_100$end[which] - combp_results_1k_100$start[which]), seq(0.01,1,by=0.01), type=1)
quants <- rbind(quants, data.frame(percentile = seq(0.01,1,by=0.01), value = temp,
                                     method = "combp",pars="combp:dist=1kb,step=100bp", chrs="Autosome"))

which <- which(combp_results_1M_100k$X.chrom == "chrX")
temp <- quantile(log10(combp_results_1M_100k$end[which] - combp_results_1M_100k$start[which]), seq(0.01,1,by=0.01), type=1)
quants <- rbind(quants, data.frame(percentile = seq(0.01,1,by=0.01), value = temp,
                                      method = "combp",pars="combp:dist=1Mb,step=100kb", chrs="chrX"))
which <- which(combp_results_1M_100k$X.chrom != "chrX" & combp_results_1M_100k$X.chrom != "chrY" )
temp <- quantile(log10(combp_results_1M_100k$end[which] - combp_results_1M_100k$start[which]), seq(0.01,1,by=0.01), type=1)
quants <- rbind(quants, data.frame(percentile = seq(0.01,1,by=0.01), value = temp,
                                     method = "combp",pars="combp:dist=1Mb,step=100kb", chrs="Autosome"))
...
filename=paste(results_dir, "/SEX_Width_Percentile_Plot.png", sep = "")
...
filename=paste(results_dir, "/SEX_Width_Percentile.csv", sep = "")
...
filename=paste(figures_dir, "/Supplemental/Supplement_neighboring_CG_cor", ".png", sep = "")  
...
filename=paste(figures_dir, "/Supplemental/Supplement_neighboring_CG_percentiles", ".png", sep = "")  
