...
write.csv(range_overlaps, file=paste(results_dir,"range_overlaps.csv",sep = ""), row.names = F )
write.csv(geneset_overlaps, file=paste(results_dir,"geneset_overlaps.csv",sep = ""), row.names = F )
```
...
for(i in 1:5){
  ...
  syn_or <- function(cg1, cg2, nt){
    ...
    return( (cg12 * cg_n12) / (cg_n1_y2 * cg_n2_y1))
  }
  syn_or_CI95 <- function(cg1, cg2, nt){
    ...
    return( paste(round(exp(left),3), "-",round(exp(right),3))   )
  }
